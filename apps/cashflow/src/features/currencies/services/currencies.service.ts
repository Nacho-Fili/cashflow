...
export class CurrenciesService {
  ...
  async update(id: string, updateCurrencyDto: UpdateCurrencyDto): Promise<CurrencyDto> {
    const currency = await this.currencyRepository.findOne({ where: { id } });
    if (!currency) {
      throw new NotFoundException(`Currency with ID ${id} not found`);
    }
    
    const updatedCurrency = this.currencyMapper.updateEntityFromDto(currency, updateCurrencyDto);
