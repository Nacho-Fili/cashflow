...
export class CreditCardsService {
  ...
  async create(createCreditCardDto: CreateCreditCardDto): Promise<CreditCardDto> {
  ...
  async update(id: string, updateCreditCardDto: UpdateCreditCardDto): Promise<CreditCardDto> {
    const creditCard = await this.creditCardRepository.findOne({ where: { id } });
    if (!creditCard) {
      throw new NotFoundException(`Credit Card with ID ${id} not found`);
    }
    
    const updatedCreditCard = this.creditCardMapper.updateEntityFromDto(creditCard, updateCreditCardDto);
    const savedCreditCard = await this.creditCardRepository.save(updatedCreditCard);
    return this.creditCardMapper.toDto(savedCreditCard);
  }
