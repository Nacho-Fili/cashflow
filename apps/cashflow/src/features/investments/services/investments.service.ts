...
export class InvestmentsService {
  ...
  async create(createDto: CreateInvestmentAccountDto): Promise<InvestmentAccountDto> {
  ...
  async update(id: string, updateDto: UpdateInvestmentAccountDto): Promise<InvestmentAccountDto> {
    const account = await this.investmentRepository.findOne({ where: { id } });
    if (!account) {
      throw new NotFoundException(`Investment Account with ID ${id} not found`);
    }
    
    const updatedAccount = this.investmentMapper.updateEntityFromDto(account, updateDto);
