...
export class IncomeSourcesService {
  ...
  async findOne(id: string): Promise<IncomeSourceDto> {
    const incomeSource = await this.incomeSourceRepository.findOne({ where: { id } });
    if (!incomeSource) {
  ...
  async create(createIncomeSourceDto: CreateIncomeSourceDto): Promise<IncomeSourceDto> {
    const incomeSource = this.incomeSourceMapper.toEntity(createIncomeSourceDto);
    const savedIncomeSource = await this.incomeSourceRepository.save(incomeSource);
    return this.incomeSourceMapper.toDto(savedIncomeSource);
  }

  async update(id: string, updateIncomeSourceDto: UpdateIncomeSourceDto): Promise<IncomeSourceDto> {
    const incomeSource = await this.incomeSourceRepository.findOne({ where: { id } });
    if (!incomeSource) {
      throw new NotFoundException(`Income Source with ID ${id} not found`);
    }
    ...
    const savedIncomeSource = await this.incomeSourceRepository.save(updatedIncomeSource);
    return this.incomeSourceMapper.toDto(savedIncomeSource);
  }
