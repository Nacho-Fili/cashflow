...
export class ExpensesService {
  ...
  async update(id: string, updateExpenseDto: UpdateExpenseDto): Promise<ExpenseDto> {
    const expense = await this.expenseRepository.findOne({ where: { id } });
    if (!expense) {
      throw new NotFoundException(`Expense with ID ${id} not found`);
    }
    
    const updatedExpense = this.expenseMapper.updateEntityFromDto(expense, updateExpenseDto);
