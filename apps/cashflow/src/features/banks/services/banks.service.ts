...
export class BanksService {
  ...
  async update(id: string, updateBankDto: UpdateBankDto): Promise<BankDto> {
    ...
    const updatedBank = this.bankMapper.updateEntityFromDto(bank, updateBankDto);
  ...
  async createBalance(id: string, createBalanceDto: CreateBalanceDto): Promise<BalanceDto> {
    ...
    try {
      ...
      console.log({ error });
      throw new InternalServerErrorException(`Failed to update bank with ID ${id} after creating balance`);
    }
